              ...
              <li key={`${s.system}-${s.en}`}>
                <a href={mapsPoiUrl(s.lat, s.lng, `${s.system} ${s.en}`)} target="_blank" rel="noopener noreferrer"
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block font-thai text-sm text-ink truncate group-hover:text-rain transition-colors">{(en ? (s.en || s.th) : (s.th || s.en)).replace(/^(BTS|MRT|ARL|SRT|Airport Rail)\s+/i, "")}</span>
                    <span className="block font-thai text-xs text-ink-faint tabular-nums">{s.km < 1 ? `${Math.round(s.km * 1000)} ${en ? "m" : "ม."}` : `${s.km.toFixed(1)} ${en ? "km" : "กม."}`}</span>
                  </span>
                </a>
              </li>
            );
